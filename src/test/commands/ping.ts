...
export default async (client: ClusterClient | Client, msg: Message) => {
	if (client instanceof ClusterClient) {
		...
		let ping = pings
			.map((d) => d.content as number)
			.reduce((acc, cur) => acc + cur, 0);

		ping = ping / pings.length;
		const shards = await client.ipc.broadcast("client.shards.size");

		await msg.sendReference(
			`Ping: \`${ping}ms\` (Cluster ID=\`${client.workerId}\`/PID=\`${process.pid}\`)\nClusters: (\`${pings.length}\`)\n${pings
				.map(
					// biome-ignore lint/suspicious/noExplicitAny: <explanation>
					(c: any) =>
						`--> Cluster (\`${c.workerId}\`): \`${c.content}\`ms (Shards: ${
							// biome-ignore lint/suspicious/noExplicitAny: <explanation>
							shards.find((r: any) => r.workerId === c.workerId)?.content
						})`,
				)
				.join("\n")}`,
		);
		...
		await msg.sendReference(
			`Ping: \`${client.shards.ping}\`ms (PID = \`${process.pid}\`)\nShards: (\`${client.shards.size}\`)\n${Array.from(
				client.shards.values(),
			)
				.map((s) => `--> (ID = ${s.id}): \`${s.ping}\`ms`)
				.join("\n")}`,
		);
	}
};
