...
export class GatewayManager extends Collection<Shard> {
	...
	public constructor(client: Client, token: string) {
		...
		if (!client || !(client instanceof Client))
			throw new Error("GatewayManager(client): client is missing or invalid.");
		if (!token || typeof token !== "string")
			throw new Error(
				"GatewayManager(token): token is missing or is not a string.",
			);
	...
	public init() {
		if (this.#client.options.sharding.connectOneShardAtTime === false) {
			for (
				let i = this.#client.options.sharding.firstShardId || 0;
				i < (this.#client.options.sharding.totalShards || 1);
				i++
			) {
				this.spawn(i);
			}
	...
	public spawn(id: number) {
		...
		if (!shard) {
			...
			shard.events.shardReady.subscribe(() => {
				const connectedShards = Array.from(this.values()).filter(
					(s) => s.ready === true,
				);

				if (
					connectedShards.length >=
					(this.#client.options.sharding.totalShards || 1)
				) {
