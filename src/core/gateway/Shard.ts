...
export class Shard {
	...
	public constructor(token: string, client: Client, id: number) {
		if (!client || !(client instanceof Client))
			throw new Error("Shard(client): client is missing or invalid.");
		if (!token || typeof token !== "string")
			throw new Error("Shard(token): token is missing or is not a string.");
	...
	public connect() {
		this.ws = new WebSocket(
			`${this.#client.options.sharding.gatewayUrl}?v=${this.#client.options.rest.apiVersion}&encoding=json`,
		);
	...
	private onMessage(msg: MessageEvent) {
		try {
			...
			switch (payload.op) {
				case 10:
					if (
						payload.data.heartbeat_interval &&
						payload.data.heartbeat_interval > 0
					) {
	...
	private async onEvent(payload: ShardPayload) {
		if (!payload)
			throw new Error("Shard.onEvent(payload): missing payload object.");
		if (payload.op !== 0)
			throw new Error("Shard.onEvent(payload): object is not a event payload");
		...
		switch (payload.t) {
			...
			case "MESSAGE_CREATE": {
				...
				if (guild) {
					const channel = guild.cache.channels.get<TextChannel>(
