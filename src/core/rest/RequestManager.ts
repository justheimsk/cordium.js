...
export class RequestManager {
	...
	public constructor(client: Client, token: string) {
		if (!token || typeof token !== "string")
			throw new Error(
				"RequestManager(token): token is missing or is not a string.",
			);
		if (!client || !(client instanceof Client))
			throw new Error("RequestManager(client): client is missing or invalid");
	...
	public request(options: RequestOptions): Promise<RequestReponse> {
		return new Promise((resolve, reject) => {
			if (!options || typeof options !== "object")
				throw new Error(
					"RequestManager.request(options): options is missing or invalid.",
				);
				...
				const req = request(requestOptions, (res) => {
					...
					res.on("end", () => {
						try {
							...
							const response = new RequestReponse(
								parsed,
								res.statusMessage || "",
								res.statusCode || null,
								requestOptions,
