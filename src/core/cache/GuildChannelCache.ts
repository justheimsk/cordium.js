...
export class GuildChannelCache extends Collection<GuildChannel> {
	...
	public constructor(client: Client, guild: Guild, channels?: any[]) {
		...
		if (!client || !(client instanceof Client))
			throw new Error(
				"GuildChannelManager(client): client is missing or invalid",
			);
		if (!guild || !(guild instanceof Guild))
			throw new Error(
				"GuildChannelManager(guild): guild is missing or invalid",
			);
		...
		if (channels) {
			...
			for (const Channel of channels) {
				...
				if (Channel.type === ChannelTypes.GUILD_TEXT)
					channel = new TextChannel(client, guild, Channel);
	...
	public async fetch(id: string) {
		if (!id)
			throw new Error(
				"GuildChannelCache.fetch(id): id is required but is missing.",
			);
		...
		if (res.data) {
			if (res.data.type === ChannelTypes.GUILD_TEXT)
				return new TextChannel(this.#client, this.guild, res.data);
			return new GuildChannel(this.#client, this.guild, res.data);
		}
	}
}
