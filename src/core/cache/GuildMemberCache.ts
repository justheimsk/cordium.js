...
export class GuildMemberCache extends Collection<GuildMember> {
  ...
  public async fetch(guildId: string, memberId: string) {
    if (!guildId) throw new Error('GuildMemberCache.fetch(guildId): guildId is required but is missing.');
    if (!memberId) throw new Error('GuildMemberCache.fetch(memberId): memberId is required but is missing.');

    return new GuildMember(this.#client, this.guild, await this.#client.rest.request({ method: 'GET', endpoint: `/guilds/${guildId}/${memberId}`, auth: true }));
  }
}
