...
import type { RequestReponse } from "../rest/RequestManager";
...
export class TextChannel extends GuildChannel {
	...
	private parseSendOptions(rawOptions: IMessageSendOptions) {
		...
		if (rawOptions?.messageReference) {
			if (!rawOptions.messageReference.messageId)
				throw new Error(
					"TextChannel.send(options.messageReference.messageId): messageId is required but is missing",
				);
			if (!rawOptions.messageReference.guildId)
				throw new Error(
					"TextChannel.send(options.messageReference.guildId): guildId is required but is missing",
				);
			options.message_reference = {};

			options.message_reference.message_id =
				rawOptions.messageReference.messageId;
			options.message_reference.guild_id = rawOptions.messageReference.guildId;
			if (rawOptions.messageReference.channelId)
				options.message_reference.channel_id =
					rawOptions.messageReference.channelId;
		}
	...
	public async deleteMessage(id: string): Promise<RequestReponse> {
		return await this.#client.rest.request({
			method: "DELETE",
			endpoint: `/channels/${this.id}/messages/${id}`,
			auth: true,
		});
	}
}
