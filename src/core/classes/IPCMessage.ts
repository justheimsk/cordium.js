...
export type IPCEvent = 'master-initial' | 'broadcast-request' | 'broadcast-response' | 'data-request' | 'data-response';
...
export class IPCMessage {
  ...
  public constructor(options: IIPCMessage) {
    ...
    if (!options.data) throw new Error('IPCMessage(options.data): data is required but is missing.');

    this.from = options.from;
    this.to = options.to || null;
    this.event = options.event;
    this.data = options.data;
    if(this.data.error == undefined || this.data.error == null) this.data.error = false;

    this.cid = options.cid;
  }

  public reply(event: IPCEvent, data: IPCData, to?: IIPCMessage['to']) {
    process.send?.(new IPCMessage({ from: process.pid, cid: this.cid, event, data, to }));
  }
