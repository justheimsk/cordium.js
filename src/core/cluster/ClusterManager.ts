...
import { type IPCData, IPCMessage } from "../classes/IPCMessage";
...
export class ClusterManager extends Collection<Worker> {
	...
	public async init(): Promise<this> {
		if (Cluster.isPrimary) {
			...
			const shards = [];
			const sep =
				(this.options.sharding?.totalShards || 2) /
				(this.options.clustering?.totalWorkers || 1);
			...
			for (let i = 0; i < chunks.length; i++) {
				...
				worker.send({
					index: i,
					shards: chunk,
					total: this.options.sharding?.totalShards || 2,
					event: "master-initial",
				});
					...
					switch (msg.event) {
						...
						case "data-request": {
							...
							if (target) {
								...
								const callback = (message: any) => {
									const response = new IPCMessage(message);
									if (!response || response.cid !== msg.cid || !response.data)
										return;
